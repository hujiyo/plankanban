# Plan Kanban —— 极简边缘呼出式计划看板

一个 Windows 桌面小工具：平时只贴在屏幕边缘一条极窄的触发条，鼠标一碰边缘/按下快捷键就滑出面板，随手管理当前目标清单。
...
2. 默认贴靠 **右边缘**，屏幕右侧会出现一条 4px 灰色细条，鼠标移上去即滑出面板。
3. 也可按 **Alt+Q** 全局呼出/收起。
4. 关闭面板：点击面板右上角的"收起"图标，或把鼠标移开面板 1.5 秒后自动收回。
...
**触发延迟**：鼠标接近边缘多久后开始呼出（0~800 ms）。默认 200 ms。

**自动收回延迟**：鼠标离开面板后多久自动收起（400~6000 ms）。默认 1500 ms。

**面板宽度**：260~460 px 可调。默认 320 px。

**主题**：跟随系统 / 浅色 / 深色。

**触发显示器**：多屏环境下可指定面板挂在哪块屏幕边缘，或选"跟随鼠标屏幕"（默认）。

**开机自启动**：勾选即写入注册表 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\PlanKanban`，下次开机自动启动。
...
| 鼠标贴边响应 | <300ms | 默认触发延迟 200ms + 260ms 滑入动画 |
...
PlanKanban/
├── Models/
│   ├── AppSettings.cs     # 设置实体（贴边、快捷键、延迟、主题、屏幕）
...
│   ├── EdgeDetector.cs    # 边缘悬停检测（低频定时器轮询鼠标位置）
...
├── Views/
│   ├── MainWindow.xaml(.cs)         # 主面板：触发条 + 收起/展开动画
...
### 2. 边缘检测实现
`EdgeDetector` 使用 `DispatcherTimer`，默认 33ms（~30Hz）逐次取 `Forms.Cursor.Position`，
比较它是否落在所选显示屏边缘的 4px 像素带内。命中后需持续 `EdgeTriggerDelayMs`
（默认 200ms）才真正触发展开，避免鼠标擦过时频繁闪烁。离开边缘自动复位计数。
性能：单次只读一个鼠标坐标 + 一次几何比较，对 CPU 几乎无压力。
...
### 4. 收起/展开动画
`Panel.RenderTransform = TranslateTransform`，`Storyboard` + `DoubleAnimation`
做 X/Y 平移，260ms `CubicEase.EaseOut`。收起到位后把 `Panel.Visibility = Collapsed`，
彻底停止子树的布局与渲染（关键节能点）。

### 5. 资源占用优化
- **收起态面板 Collapsed**：WPF 在 Visibility=Collapsed 时跳过 measure/arrange/render，GPU 几乎为 0。
- **边缘检测降频余地**：可在空闲时进一步降低 timer 频率（当前 30Hz 已足够低）。
...
- **不支持 Windows 7/8**：.NET 8 最低要求 Win10。
